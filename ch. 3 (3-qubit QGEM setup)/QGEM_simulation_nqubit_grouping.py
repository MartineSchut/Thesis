...
groups_sext, num_cliques = tpb_group(pauli_to_measure_sext)
...
groups_quin, num_cliques = tpb_group(pauli_to_measure_quin)
...
groups_quad, num_cliques = tpb_group(pauli_to_measure_quad)
...
groups_triple, num_cliques = tpb_group(pauli_to_measure_triple)
...
groups_double, num_cliques = tpb_group(pauli_to_measure_double)
...
def shots_weighting_dict(color_ave_weight_list, shots_budget):
    shots_per_term = []
    tots = 0.0
    for val in color_ave_weight_list:
        tots = tots + abs(val)
    for val in color_ave_weight_list:
...
def get_list_metrics_dict(measurement_list, EW_weight_list, shots_per_color, color_list):
    term_variance = []
    term_expectation = []
    for term_idx in range(len(measurement_list)):
        color = int(color_list[term_idx] - 1)
        shots = shots_per_color[color]
        variance_term = 0.0
        if shots >1:
    ...
    for color_id in range(1, num_cliques + 1):
        tot_color_id = 0.0
        for i in range(len(EW_weight_list)):
            if color_list[i] == color_id:
                tot_color_id += 1.0
        color_count.append(tot_color_id)
    tot_virtual_shots = 0.0

    for color_id in range(num_cliques):
        tot_virtual_shots += color_count[color_id]*shots_per_color[color_id]
    ave_virt_shots = tot_virtual_shots/len(EW_weight_list)

    average_shots_sqrt = np.sqrt(ave_virt_shots)
...
def run_experiment_dict(q_state, max_shots_count, EW_weight_list, EW_ave_color_list, color_list, cumul_list, e_vals_list):
    shots_per_color = shots_weighting_dict(EW_ave_color_list, max_shots_count)
    measurement_list = []
    for term_idx in range(len(EW_weight_list)):
        measurement_list.append([])
        color = int(color_list[term_idx] - 1)
...
def make_color_list(group_map, ops_list, num_cliques):
    color_list = np.zeros([len(ops_list)])
    for elem_id in range(len(ops_list)):
        for color_id in range(1, num_cliques + 1):
            if ops_list[elem_id] in group_map[str(color_id)]: 
                color_list[elem_id] = color_id
    return color_list

def get_smooth_confidence_grouped(q_state, shot_budgets, EW_test, repeat, physics_values):
    paulis = all_letters(len(q_state.qudit_list))
    basis = gen_paulis_tensors(2, len(q_state.qudit_list))
    weights_EW = EW_test.decomposition(basis)
    ops_list = [paulis[i] for i in range(len(weights_EW)) if weights_EW[i] != 0]
    ops_list = ops_list[1:]

    EW_term_list, EW_weight_list, ID_weight = get_reduced_term_list(EW_test, q_state.dimension, len(q_state.qudit_list))
    group_map, num_cliques = tpb_group(ops_list)
    color_list = make_color_list(group_map, ops_list, num_cliques)
    color_ave_weight_list = []
    color_count = []
    for color_id in range(1, num_cliques + 1):
        tot_weight = 0.0
        tot_color_id = 0.0
        for i in range(len(ops_list)):
            if color_list[i] == color_id:
                tot_color_id += 1.0
                tot_weight += abs(EW_weight_list[i])
        color_ave_weight_list.append(tot_weight/tot_color_id)
        color_count.append(tot_color_id)
    cumul_list = []
    e_vals_list = []
    ...
    for _ in range(repeat):
        print('...............Iteration: ', _)
        measurement_list = run_experiment_dict(q_state, max(shot_budgets), EW_weight_list, color_ave_weight_list, color_list, cumul_list, e_vals_list) #TODO update func
        for shots_idx in range(len(shot_budgets)):
            shots_per_color = shots_weighting_dict(color_ave_weight_list, shot_budgets[shots_idx])
            ave, std_err = get_list_metrics_dict(measurement_list, EW_weight_list, shots_per_color, color_list)
            ave = ave + ID_weight
            t_stats = -1.0*ave/(std_err)
            tot_virtual_shots = 0.0
            for color_id in range(num_cliques):
                tot_virtual_shots += color_count[color_id]*shots_per_color[color_id]
            ave_virt_shots = tot_virtual_shots/len(EW_term_list)
            adj_shots_per_term = ave_virt_shots - 1.0
            pval = stats.t.sf(np.abs(t_stats), adj_shots_per_term) #One sided test
            ave_by_shot_nums[shots_idx] += np.real(ave/float(repeat))
            confidence_by_shot_nums[shots_idx] += np.real((1.0 - pval)/float(repeat))
...
def plot_confidence_grouped(q_state_1, q_state_2, shot_budgets, EW_1, EW_2, repeat, physics_values):
    begining = time.time()
    target_val = EW_1.apply_to_state(q_state_1, physics_values)  

    ave_2, confidence_2 = get_smooth_confidence_grouped(q_state_2, shot_budgets, EW_2, repeat, physics_values)
    ave_1, confidence_1 = get_smooth_confidence(q_state_1, shot_budgets, EW_1, repeat, physics_values)
...
q_state_1 = quantum_state_triple
...
plot_confidence_grouped(q_state_1, q_state_2, shot_budgets, EW_1, EW_2, repeat, physics_values)
